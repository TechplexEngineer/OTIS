...
class Example_obsession_version extends Controller {

	function Example_obsession_version()
	{
		parent::Controller();	
	}
	...
	function index()
	{
		...
		if($this->session->flashdata('error')) {
			echo '<div class="error" style="background-color:#FF9999;">';
				echo $this->session->flashdata('error_text');
			echo '</div>';
		} elseif($this->session->flashdata('success')) {
			echo '<div class="success" style="background-color:#66FFCC;">';
		...
		echo '<div id="create">';
			...
			echo '<form action="' . site_url('/example_obsession_version/create/') . '" method="post">';
				
				echo '<label for="create_username">Username:</label>';
				echo '<input type="text" id="create_username" name="create_username" value="' . htmlspecialchars($this->session->flashdata('create_username')) . '" /><br />';

				echo '<label for="create_password">Password:</label>';
				echo '<input type="password" id="create_password" name="create_password" value="' . htmlspecialchars($this->session->flashdata('create_password')) . '" /><br />';
		...
		if(!$this->session->userdata('logged_in')) {
			echo '<div id="login">';
				echo '<h3>Login</h3>';
				echo '<form action="' . site_url('/example_obsession_version/login/') . '" method="post">';
					
					echo '<label for="login_username">Username:</label>';
					echo '<input type="text" id="login_username" name="login_username" value="' . htmlspecialchars($this->session->flashdata('login_username')) . '" /><br />';
	
					echo '<label for="login_password">Password:</label>';
					echo '<input type="password" id="login_password" name="login_password" value="' . htmlspecialchars($this->session->flashdata('login_password')) . '" /><br />';
			...
			echo '<div id="logut">';
				echo '<h3>Logut</h3>';
				echo '<a href="' . site_url('/example_obsession_version/logout/') . '">Click here to logout.</a>';
			echo '</div>';
			echo '<hr />';
					...
					echo '<table>';
						...
						foreach($user_array as $ua) {
							echo '<tr>';
								...
								echo '</td>';
								echo '<td>';
									echo '<a href="' . site_url('/example_obsession_version/delete/' . $ua['id']) . '" onclick="return confirm(\'Are you sure you want to delete this user?\')">Delete</a>';
								echo '</td>';
							echo '</tr>';
						}
					echo '</table>';
	...
	function create()
	{
		...
		if ($this->validation->run() == false) {
			...
			/**/
			redirect('/example_obsession_version/');			
		} else {
			//Create account
